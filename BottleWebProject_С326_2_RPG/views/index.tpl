% rebase('layout.tpl', title='Главная', year=year)
<link rel="stylesheet" type="text/css" href="/static/content/main.css" />
<main class="graph-tasks-wrapper">
    
    <div class="page-header">
        <h1>Варианты задач <small>графовое моделирование</small></h1>
        <p class="lead">Выберите задачу для изучения алгоритма или перехода к интерактивной демонстрации</p>
    </div>

    <div class="tasks-grid">
        
        <!-- Вариант 1 -->
        <section class="task-card">
            <div class="card-header">
                <span class="badge">BFS</span>
                <h3>Вирус</h3>
            </div>
            <div class="card-body">
                <p>Моделирование распространения заражения в неориентированном графе.</p>
                <ul class="params-list">
                    <li><strong>Старт:</strong> <code>K</code> заражённых вершин</li>
                    <li><strong>Шаг:</strong> заражение всех соседей</li>
                    <li><strong>Модификация:</strong> вероятность <code>p &lt; 1</code></li>
                </ul>
            </div>
            <div class="card-footer">
                <a href="/bfs" class="btn btn-blue">Запустить модуль</a>
            </div>
        </section>

        <!-- Вариант 2 -->
        <section class="task-card">
            <div class="card-header">
                <span class="badge">TSP</span>
                <h3>Экскурсии</h3>
            </div>
            <div class="card-body">
                <p>Оптимальный маршрут туриста по достопримечательностям города.</p>
                <ul class="params-list">
                    <li><strong>Вершин:</strong> <code>N ≤ 50</code></li>
                    <li><strong>Посетить:</strong> <code>M ≤ 8</code></li>
                    <li><strong>Финиш:</strong> возврат в отель (вершина <code>K</code>)</li>
                </ul>
            </div>
            <div class="card-footer">
                <a href="/tsp" class="btn btn-cyan">Запустить модуль</a>
            </div>
        </section>

        <!-- Вариант 3 -->
        <section class="task-card">
            <div class="card-header">
                <span class="badge">DFS</span>
                <h3>Блокчейн</h3>
            </div>
            <div class="card-body">
                <p>Анализ цепочек транзакций для выявления подозрительных паттернов.</p>
                <ul class="params-list">
                    <li><strong>Граф:</strong> ориентированный (кошельки → переводы)</li>
                    <li><strong>Цель:</strong> самый длинный путь без циклов</li>
                    <li><strong>Применение:</strong> поиск отмывания средств</li>
                </ul>
            </div>
            <div class="card-footer">
                <a href="/dfs" class="btn btn-teal">Запустить модуль</a>
            </div>
        </section>

    </div>

    <!-- Справка -->
    <div class="algorithm-details">
        <div class="info-box">
            <h4>📖 Справка по алгоритмам</h4>
            <dl>
                <dt>BFS</dt>
                <dd>Поиск в ширину — обход графа слой за слоем, идеален для поиска кратчайших путей в невзвешенных графах.</dd>
                <dt>TSP</dt>
                <dd>Задача коммивояжёра — поиск гамильтонова цикла минимального веса; для M≤8 решается полным перебором или динамикой по подмножествам.</dd>
                <dt>DFS</dt>
                <dd>Поиск в глубину — рекурсивный обход с отслеживанием посещённых вершин, подходит для поиска путей и циклов.</dd>
            </dl>
        </div>
    </div>

</main>