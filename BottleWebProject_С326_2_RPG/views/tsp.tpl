...
% cls_sites = 'is-error' if _errors.get('sites') else ''
  ...
  <form method="POST" action="/excursion" enctype="multipart/form-data" novalidate>
    <input type="hidden" id="buttonAction" name="action" value="solve">

    <div class="tsp-card">
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; margin-bottom: 20px;">
        <h2 style="margin: 0;">Параметры графа</h2>
        
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <button type="submit" onclick="document.getElementById('buttonAction').value='generate';" class="btn-generate-tsp" style="margin: 0; height: 42px;">
            🎲 Случайный граф
          </button>
          
          <label class="btn-generate-tsp" style="margin: 0; background-color: #f1f5f9; color: #0f172a; border: 1px solid #cbd5e1; display: inline-flex; align-items: center; justify-content: center; height: 42px; cursor: pointer; font-weight: 700;">
            📂 Загрузить из .txt
      ...
      <div class="fields-row">
        ...
        <div class="field">
          <label for="m">Достопримечательности M <span class="hint">≤ 8</span></label>
        ...
        <div class="field">
          <label for="k">Вершина отеля K</label>
      ...
      <div class="field field-full">
        <label for="edges">Рёбра графа <span class="hint">по одному на строку: u v w</span></label>
        <textarea id="edges" name="edges" placeholder="1 2 5&#10;2 3 3&#10;1 3 10" style="height: 160px;" class="{{cls_edges}}">{{val_edges}}</textarea>
      ...
      <div class="field field-full">
        <label for="sites">Достопримечательности <span class="hint">через пробел или запятую</span></label>
      ...
      <button type="submit" onclick="document.getElementById('buttonAction').value='solve';" class="btn-primary-tsp" style="width: 100%; padding: 14px; font-size: 16px;">Найти оптимальный маршрут</button>
    </div>
  </form>
  ...
  <div class="tsp-card">
    ...
    <div class="canvas-container" id="canvas-container">
      % if _svg:
        {{!_svg}}
      % else:
        <div class="canvas-placeholder">Заполните поля параметров и рёбер графа для генерации визуальной схемы</div>
      % end
    </div>
  </div>
  ...
  % if _result is not None:
    % if _result['reachable']:
      <div class="result-card">
        ...
        <p class="total-time">
          Суммарное время: <strong>{{_result['total_time']}}</strong> ед.
        </p>
      </div>
    % else:
      <div class="result-error">
        Маршрут не найден — граф несвязен или не все вершины достижимы из отеля.
      </div>
    % end
  % end
